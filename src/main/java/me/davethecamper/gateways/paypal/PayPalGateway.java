package me.davethecamper.gateways.paypal;
...
public class PayPalGateway implements CashShopGateway {
	...
	public TransactionInfo generateTransaction(ProductInfo product, PlayerInfo player) {
		...
		purchaseUnits.add(
				new PurchaseUnitRequest()
					.softDescriptor("Cash")
					.amountWithBreakdown(
							new AmountWithBreakdown().currencyCode(product.getCurrency()).value(df.format(product.getAmount()))));

		orderRequest.purchaseUnits(purchaseUnits);
		orderRequest.applicationContext(this.getApplicationContext());
	...
	private ApplicationContext getApplicationContext() {
		return new ApplicationContext()
				.brandName("AntiSocial")
				.returnUrl("https://discord.gg/Nx2hSZn")
				.cancelUrl("https://discord.gg/Nx2hSZn");
	}

	@Override
	public TransactionResponse verifyTransaction(String token) {

		if(processOrder(token).equals(TransactionResponse.APPROVED)) {
	...
	private TransactionResponse processOrder(String token) {
	...
	private void testCredentials() {
		generateTransaction(new ProductInfo(1, "Testing Credentials", "USD"), null);
	}


	@Override
	public String getColoredDisplayName() {
		return "§9§lPay§b§lPal";
	}
